...
class UserController {
  async create(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const { name, email, password, mobile, roleId, branchId, branchIds } = req.body;
      ...
      const user = await userService.create(
        { name, email, password, mobile, role_id: roleId, branch_id: branchId, branchIds } as any,
        userId
      );
  ...
  async update(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const { id } = req.params;
      const { name, email, mobile, roleId, branchId, branchIds, status } = req.body;
      const userId = req.user?.userId;
      ...
      const user = await userService.update(Number(id), { name, email, mobile, role_id: roleId, branch_id: branchId, branchIds } as any, userId);
